...
char *argv[];
{
  ...
  /*
  qmax=0; LOOP(NX,NYP,NZ){ if(qmax<q[k][j][i])qmax=q[k][j][i];
            fprintf(stderr,"%g",q[k][j][i]);
             };
  ...
  while(t<=tmax){
    ...
    /* calculate thermal forcing term H and dH/dy */
    ...
    */
      ...
      writef2(&ubar[0][0],1,NYP,NZ);
      writef2(&qbary[0][0],1,NYP,NZ);
      writef2(&phi[0],1,NYP,1);
      /*
    ...
    case 0:
      ...
      LOOPYZ(NYP,NZ) ubar[k][j] += dt*dubar2dt[k][j];
      t += dt;tcount++;
      icase=1;
      break;
    case 1:
      LOOP(NX,NYP,NZ) q[k][j][i] += dt2rk*(dqdt[k][j][i]-dq2dt[k][j][i]);
      LOOPYZ(NYP,NZ) ubar[k][j] += dt2rk*(dubardt[k][j]-dubar2dt[k][j]);
      icase=-1;
      break;
    case -1:
      LOOP(NX,NYP,NZ) q[k][j][i] += dt*dq1dt[k][j][i];
      LOOPYZ(NYP,NZ) ubar[k][j] += dt*dubar1dt[k][j];
      t += dt;tcount++;
      icase=3;
      break;
    case 3:
      LOOP(NX,NYP,NZ) q[k][j][i] += dt2rk*(dqdt[k][j][i]-dq1dt[k][j][i]);
      LOOPYZ(NYP,NZ) ubar[k][j] += dt2rk*(dubardt[k][j]-dubar1dt[k][j]);
      icase= -2;
      break;
    case -2:
      LOOP(NX,NYP,NZ) {
	...
	dq1dt[k][j][i]=dqdt[k][j][i];
      ...
      LOOPYZ(NYP,NZ) {
	ubar[k][j] += dt0*dubardt[k][j]+dt1*dubar1dt[k][j]+dt2*dubar2dt[k][j];
	dubar2dt[k][j]=dubar1dt[k][j];
	dubar1dt[k][j]=dubardt[k][j];
      };
...
ARRAY(f); /* transform of rhs (output) */
{
  ...
  LOOP(NX,NYP,NZ) f[k][j][i] = -(u[k][j][i]+ubar[k][j])*(qreal[k][j][i]+topo[j][i]*(k==1));
  sinetrans(f,NX,NYP,NZ);
  ddx(f,k0,NX,NYP,NZ);
  
  LOOP(NX,NYP,NZ) work[k][j][i] = -v[k][j][i]*(qreal[k][j][i]+topo[j][i]*(k==1));
  ...
  //LOOP(NX,NYP,NZ) work[1][j][i] += u[1][j][i]*topox[j][i];
  sinetrans(work,NX,NYP,NZ);
  LOOP(NX,NYP,NZ) f[k][j][i] += work[k][j][i];
  
  /* friction */
  LOOPXY(NX,NYP) {
    f[0][j][i] -= r1*fr1*(psi[1][j][i]-psi[0][j][i])
                +kappa*wv[j][i]*q[0][j][i];
    f[1][j][i] -= r1*fr2*(psi[0][j][i]-psi[1][j][i]) 
                +r*(q[1][j][i] +fr2*(psi[1][j][i]-psi[0][j][i]))
...
VECTOR(f);
{
  ...
  LOOPY(NYP) {
    ru[0][j] = forcing[0][j];
    ru[1][j] = -r*ubar[1][j]; // why is there the u1-u2 terms in both?
  };
  LOOPYZ(NYP,NZ) ru[k][j] += vpqpbar[k][j]-kappa*(qbary[k][j]-beta);
  
  LOOPY(NYP) phi[j]= ru[0][j]-ru[1][j]+Hy[j]+r1*(ubar[0][j] -ubar[1][j]);// + Hy[j];//added H term
  sinft(phi, NY, 0);
